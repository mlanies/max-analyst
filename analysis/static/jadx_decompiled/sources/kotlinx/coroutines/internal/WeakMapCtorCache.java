...
final class WeakMapCtorCache extends CtorCache {
    ...
    public m56 get(Class<? extends Throwable> key) {
        ...
        lock.lock();
        try {
            m56 m56Var = exceptionCtors.get(key);
            if (m56Var != null) {
                return m56Var;
            }
            ...
            writeLock.lock();
            try {
                WeakHashMap<Class<? extends Throwable>, m56> weakHashMap = exceptionCtors;
                m56 m56Var2 = weakHashMap.get(key);
                if (m56Var2 != null) {
                    return m56Var2;
                }
                m56 m56VarCreateConstructor = ExceptionsConstructorKt.createConstructor(key);
                weakHashMap.put(key, m56VarCreateConstructor);
                while (i < readHoldCount) {
                    lock2.lock();
                    i++;
                }
                writeLock.unlock();
                return m56VarCreateConstructor;
            } finally {
                while (i < readHoldCount) {
                    lock2.lock();
                    i++;
